...
export const stockService = {
    ...
    async ensureStockMovement(movement: StockMovement): Promise<{ stockMovement: StockMovement | null; error?: string }> {
        ...
        try {
            ...
            await auditService.logAction({
                action: AUDIT_EVENT_CATALOG.stock.movementCreated.action,
                entityType: AUDIT_EVENT_CATALOG.stock.movementCreated.entityType,
                entityId: createdMovement.id,
                oldData: null,
                newData: createdMovement,
            });
    ...
    async updateStockMovement(stockId: string, stock: StockMovement): Promise<boolean> {
        ...
        try {
            ...
            await auditService.logAction({
                action: AUDIT_EVENT_CATALOG.stock.movementUpdated.action,
                entityType: AUDIT_EVENT_CATALOG.stock.movementUpdated.entityType,
                entityId: stockId,
                oldData: oldMovement ?? null,
                newData: stock,
            });
    ...
    async deleteStockMovement(stockId: string): Promise<boolean> {
        ...
        try {
            ...
            await auditService.logAction({
                action: AUDIT_EVENT_CATALOG.stock.movementDeleted.action,
                entityType: AUDIT_EVENT_CATALOG.stock.movementDeleted.entityType,
                entityId: stockId,
                oldData: movement ?? null,
                newData: null,
            });
