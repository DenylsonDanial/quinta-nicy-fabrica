...
function validatePurchaseCompliance(input: PurchaseComplianceInput): string | null {
  ...
  if (!supplierId) {
    return 'Compliance fiscal: fornecedor obrigatorio.';
  }
  if (!invoiceNumber) {
    return 'Compliance fiscal: numero de fatura obrigatorio.';
  }
  if (!supplierName) {
    return 'Compliance fiscal: nome do fornecedor obrigatorio.';
  }
...
export const purchaseService = {
  ...
  async createPurchase(purchase: Purchase, updateStock = false): Promise<{ purchase: Purchase | null }> {
    ...
    try {
      ...
      if (complianceError) {
        console.warn(`[purchaseService] createPurchase bloqueada: ${complianceError}`);
  ...
  async updatePurchase(
    ...
    try {
      ...
      const complianceError = validatePurchaseCompliance({
        ...
        supplierName: updates.supplierName ?? oldRow?.supplier_name,
        invoiceNumber: updates.invoiceNumber ?? oldRow?.invoice_number,
      });
      if (complianceError) {
        console.warn(`[purchaseService] updatePurchase bloqueada: ${complianceError}`);
        return false;
      }
