...
export const hasMinimumPurchaseCompliance = (purchase: Partial<Purchase>): boolean => {
  const supplierId = String(purchase.supplierId ?? '').trim();
  const invoiceNumber = String(purchase.invoiceNumber ?? '').trim();
  return supplierId.length > 0 && invoiceNumber.length > 0;
};

export const matchesPurchaseCompositeFilters = (
  purchase: Purchase,
  supplierFilter: string,
  invoiceFilter: string,
): boolean => {
  const normalizedSupplier = normalizeForSearch(supplierFilter);
  const normalizedInvoice = normalizeForSearch(invoiceFilter);
  const purchaseSupplier = normalizeForSearch(purchase.supplierName || purchase.supplierLocationName || '');
  const purchaseInvoice = normalizeForSearch(purchase.invoiceNumber || '');

  const supplierMatches = !normalizedSupplier || purchaseSupplier.includes(normalizedSupplier);
  const invoiceMatches = !normalizedInvoice || purchaseInvoice.includes(normalizedInvoice);
  return supplierMatches && invoiceMatches;
};
...
export const Purchases: React.FC<PurchasesProps> = ({
  ...
  const [invoiceFilterQuery, setInvoiceFilterQuery] = useState('');
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, supplierFilterQuery, invoiceFilterQuery, selectedPeriod, customStartDate, customEndDate, filterPaymentStatus]);
...
<html>
  ...
  const handleSubmitPurchase = async () => {
    ...
    if (!hasMinimumPurchaseCompliance(purchase)) {
      showToast('Fornecedor e número de fatura são obrigatórios para submeter a compra', 'error');
      return;
    }
  ...
  const filteredAndSortedPurchases = useMemo(() => {
    ...
    filtered = filtered.filter((p) => matchesPurchaseCompositeFilters(p, supplierFilterQuery, invoiceFilterQuery));
    ...
    return filtered;
  }, [purchases, searchQuery, supplierFilterQuery, invoiceFilterQuery, selectedPeriod, customStartDate, customEndDate, filterPaymentStatus, sortField, sortDirection]);
        ...
        <>
          ...
          <FilterBar isStickyOnMobile={isMobile} stickyTopClassName="top-0">
            ...
            <input
              ...
              className="h-8 px-2 text-xs rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 min-w-[110px] max-w-[160px]"
            />
            ...
            {(searchQuery || supplierFilterQuery || invoiceFilterQuery || selectedPeriod !== 'thisMonth' || customStartDate || customEndDate || filterPaymentStatus !== 'ALL') && (
              <button
                onClick={() => {
                  setSearchQuery('');
                  setSupplierFilterQuery('');
                  setInvoiceFilterQuery('');
