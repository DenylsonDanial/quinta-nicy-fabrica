...
import { isKnownAuditAction, isKnownAuditEntityType } from './auditEvents';
...
export class AuditService {
    ...
    static async logAction(params: {
        ...
        try {
            if (params.action.includes('.') && !isKnownAuditAction(params.action)) {
                return { success: false, error: `Ação de auditoria desconhecida: ${params.action}` };
            }
            if (params.entityType.includes('_') && !isKnownAuditEntityType(params.entityType)) {
                return { success: false, error: `Tipo de entidade desconhecido: ${params.entityType}` };
            }
